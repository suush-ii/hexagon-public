...
export function tradeSql(size: number, page: number) {
	...
	return db
		.select({
			...
			offering: sql`coalesce(json_agg(DISTINCT jsonb_build_object('itemid', ${inventoryTable.itemid}, 'inventoryid', ${inventoryTable.inventoryid}, 'assetname', ${assetTable.assetname}, 'serial', ${inventoryTable.serialid}, 'sales', ${assetTable.sales}, 'recentaverageprice', ${assetTable.recentaverageprice}, 'originalprice', ${assetTable.price})) FILTER (WHERE ${inventoryTable.itemid} IS NOT NULL), '[]'::json)`,
			requesting: sql`coalesce(json_agg(DISTINCT jsonb_build_object('itemid', ${requesting.itemid}, 'inventoryid', ${requesting.inventoryid}, 'assetname', ${requestingAsset.assetname}, 'serial', ${requesting.serialid}, 'sales', ${requestingAsset.sales}, 'recentaverageprice', ${requestingAsset.recentaverageprice}, 'originalprice', ${requestingAsset.price})) FILTER (WHERE ${requesting.itemid} IS NOT NULL), '[]'::json)`,
