<script lang="ts">
    ...
    function scroll(direction: "left" | "right"){ // ewww svelte doesn't let you bind:scroll so we have to hack it :(
...
<Button class="h-full" size="icon" variant="outline" on:click={() => {scroll("left")}} disabled={leftButtonDisabled}><ChevronLeft className="h-2 w-2"/></Button>
...
<Button class="h-full" size="icon" variant="outline" on:click={() => {scroll("right")}} disabled={rightButtonDisabled}><ChevronRight className="h-2 w-2"/></Button>
